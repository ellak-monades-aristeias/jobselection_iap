...
<head>
	...
	<title>Registration Information | Job Selection</title>
		...
		<div id="content" class="site-content" role="main">
				...
				<article id="post-4" class="post-4 page type-page status-publish hentry">
					<header class="entry-header">
						...
						<h1 class="entry-title">Registration Information</h1>
					</header><!-- .entry-header -->
...
//lines 99-103: Establish database connection
...
//lines 106-187: Selects all the saved information for the entity company
if ( isset($_GET["t"]) ) {
	...
	$sql1= "SELECT * FROM user_connection WHERE representative_token= '" .$rep_system_t ."'";
	...
	if ($sql1_result->num_rows ==1){
				...
				$emp_system_t = $row["employee_token"];
				$comp_system_t = $row["company_token"];
	...
	$emp_sql= "select * from wp_users where token= '". $emp_system_t ."'";
	...
	if ($emp_result->num_rows ==1){
				...
				$emp_tel = $row["telephone_number"];
				$emp_user_email = $row["user_email"];
			
	}

	require_once('wp-config.php');

	$rep_sql= "select * from wp_users where token= '". $rep_system_t  ."'";
	$rep_result= mysqli_query($con,$rep_sql); 

	if ($rep_result->num_rows ==1){
			
				$row = $rep_result->fetch_assoc();
			
				$rep_name = $row["name"];
				$rep_surname = $row["surname"];
				$rep_tel = $row["telephone_number"];
				$rep_user_email = $row["user_email"];
			
	}

	require_once('wp-config.php');

	$comp_sql= "select * from companies where token= '". $comp_system_t ."'";
	$comp_result= mysqli_query($con,$comp_sql); 

	if ($comp_result->num_rows ==1){
			
				$row = $comp_result->fetch_assoc();
			
				$host_category = $row["host_category"];
				$field_of_activity = $row["field_of_activity"];
				$comp_name = $row["company_name"];
				$comp_tel = $row["company_telephone"];
				$fax = $row["fax"];
				$comp_email = $row["company_email"];
				$website = $row["website"];
				$number_of_employees = $row["number_of_employees"];
				$contact_person_name = $row["contact_person_name"];
				$contact_person_surname = $row["contact_person_name"];
				$contact_person_tel = $row["contact_person_telephone"];
				$contact_person_mobile = $row["conctact_person_mobile"];
				$contact_person_email = $row["contact_person_email"];
				$deputy_name = $row["deputy_name"];
				$deputy_surname = $row["deputy_surname"];
				$deputy_tel = $row["deputy_telephone"];
				$deputy_mobile = $row["deputy_mobile"];
				$deputy_email = $row["deputy_email"];
			
			
	}

//lines 190-242: Prints on the screen all the information so to be checked by the user-representative in order to continue with the registration process
	//echo "The information provided in the first part of the \" " . $comp_name ." \" 's registration were the following: <br/><br/><br/>";
	echo "<p>The following information has been provided as regards as the company's registration in the Internship Application Platform.</p>";
	echo "<p>Please validate the information and proceed with STORK 2.0 Authentication on Behalf of the company to finalize the registration.</p><br/>";
	echo "<u>User prepared the first part of the registration:</u><br/>";
	echo "First Name:" . $emp_name ."<br/>";
	echo "Surname:" . $emp_surname ."<br/>";
	echo "Telephone Number:" . $emp_tel ."<br/>";
	echo "Email:" . $emp_user_email ."<br/><br/>";
	
	echo "<u>Representative's information given in the first part of the registration:</u> <br/>";
	echo "First Name:" . $rep_name ."<br/>";
	echo "Surname:" . $rep_surname ."<br/>";
	echo "Telephone Number:" . $rep_tel ."<br/>";
	echo "Email:" . $rep_user_email ."<br/><br/>";
	
	echo "<u>Company's information given in the first part of the registration:</u> <br/>";
	echo "Host Category:" . $host_category ."<br/>";
	echo "Field Of Activity:" . $field_of_activity ."<br/>";
	echo "Name:" . $comp_name ."<br/>";
	echo "Telephone Number:" . $comp_tel ."<br/>";
	echo "Fax:" .$fax ."<br/>";
	echo "Email:" .$comp_email ."<br/>";
	echo "Website:" .$website ."<br/>";
	echo "Number Of Employees:" .$number_of_employees ."<br/>";
	
	/**
	echo "Contact Person's Name:" .$contact_person_name ."<br/>";
	echo "Contact Person's Surname:" .$contact_person_surname ."<br/>";
	echo "Contact Person's Telephone Number:" .$contact_person_tel ."<br/>";
	echo "Contact Person's Mobile Number:" .$contact_person_mobile ."<br/>";
	echo "Contact Person's Email:" .$contact_person_email ."<br/>";
	echo "Deputy Contact Person's Name:" .$deputy_name ."<br/>";
	echo "Deputy Contact Person's Surname:" .$deputy_surname ."<br/>";
	echo "Deputy Contact Person's Telephone Number:" .$deputy_tel ."<br/>";
	echo "Deputy Contact Person's Mobile Number:" .$deputy_mobile ."<br/>";
	echo "Deputy Contact Person's Email:" .$deputy_email ."<br/>";
	*/
	
	echo "<br/><br/>";
	
	echo "<p>Please click \"Next\" below in order to Authenticate on Behalf of the company via STORK 2.0 and continue the process.</p><br/>";
	
	echo "<form action='representative_SS_redirection.php?t=" .$rep_system_t ."' method='post' name='send_email'> <table>";
	echo "<table>";
	echo "<tr> <input type='submit' value='Next' /> </tr>";
	echo "<tr>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</tr>";
	echo " <tr> <button type='button' onclick='location.href = `http://stork-ap.aegean.gr/JobSelection/cancel_form.php`;'>Cancel</button> </tr>";
	echo "</table> </form>";
}
